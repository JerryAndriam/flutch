...
app.use((req, res, next) => {
    ...
    if ((req.originalUrl || req.url || "").startsWith("/api/webhook"))
        return next();
    ...
    return res
        .status(403)
        .json({ error: "Requête cross-origin non autorisée" });
});
...
app.use((req, _res, next) => {
    if (req.session && req.session.userId && req.session.cookie) {
        req.session.cookie.expires = new Date(
            Date.now() + config.SESSION_MAX_AGE_MS,
        );
    }
    next();
});
...
async function runDailyIntegrity() {
    ...
    if (report.issues.length)
        logger.warn("⚠️ Intégrité DB : " + JSON.stringify(report.issues));
    else logger.info("✅ Intégrité DB : tout est clean");
}

async function processWebhookJob(payload, eventType) {
    const { current, dealId, stageId, status, bienStageId, acqStageId } =
        payload;
    ...
    if (
        ...
        await pipedrive.archiveDeal(dealId);
        return;
    }
    ...
    if (isBienStage && status === "open") {
        await pipedrive.archiveDeal(dealId);
        await pipedrive.syncSingleBien(current, config.PIPEDRIVE_API_TOKEN);
    } else if (isAcqStage && status === "open") {
        await pipedrive.archiveDeal(dealId);
        await pipedrive.syncSingleAcquereur(current);
    } else {
        ...
        if (existingBien.length || existingAcq.length) {
            await pipedrive.archiveDeal(dealId);
            logger.info(
                `⚡ Consumer: deal #${dealId} a quitté les étapes cibles → archivé`,
            );
        }
    }
}
...
async function startServer() {
    app.listen(config.PORT, async () => {
        ...
        logger.info(
            `   Biens         : pipeline "${config.BIENS_PIPELINE}" · étape "${config.BIENS_STAGE}"`,
        );
        logger.info(
            `   Acquéreurs    : pipeline "${config.ACQUEREURS_PIPELINE}" · étape "${config.ACQUEREURS_STAGE}"`,
        );
        logger.info(
            `   Auto-sync     : ${config.AUTO_SYNC_MINUTES > 0 ? `toutes les ${config.AUTO_SYNC_MINUTES} min` : "désactivé"}`,
        );
        logger.info(`   Webhooks      : asynchrone (queue PostgreSQL)`);
        logger.info(
            `   Intégrité     : quotidienne à ${config.DAILY_INTEGRITY_HOUR}h du matin\n`,
        );
        ...
        if (config.PIPEDRIVE_API_TOKEN) {
            const baseUrl = config.REPLIT_DEV_DOMAIN
                ...
                : config.REPL_SLUG
                  ? `https://${config.REPL_SLUG}.${config.REPL_OWNER}.repl.co`
                  : null;
            resolveStageIds()
                .then(() => {
                    if (baseUrl) {
                        ...
                        logger.warn(
                            "⚠️ Pas de domaine public détecté — webhook non enregistré",
                        );
                    }
                })
                .catch((err) =>
                    logger.error(
                        "Failed to resolve Pipedrive stage IDs at startup",
                        err,
                    ),
                );
        }
        ...
        // Webhook Consumer : traitement asynchrone via queue PostgreSQL
        const pipedrive = require("./pipedrive");
        startConsumer(processWebhookJob, {
            concurrency: 5,
            batchSize: 10,
            intervalMs: 1000,
        });
        ...
        if (nextIntegrity <= now)
            nextIntegrity.setDate(nextIntegrity.getDate() + 1);
