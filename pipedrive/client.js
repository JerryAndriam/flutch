...
function isRetryablePdError(err) {
    ...
    if (/ETIMEDOUT|ECONNRESET|EAI_AGAIN|ENOTFOUND|ECONNREFUSED/.test(m))
        return true;
    return false;
}
    ...
    return withRetry(
        async () => {
            const res = await fetch(
                `${PIPEDRIVE_BASE}${path}${sep}api_token=${apiToken}`,
            );
        ...
        {
            retries: 3,
            label: `pdGet ${path.split("?")[0]}`,
            shouldRetry: isRetryablePdError,
        },
    );
}
