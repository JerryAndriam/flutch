...
async function syncSingleBien(deal, apiToken) {
    ...
    const { rows: existing } = await pool.query(
        "SELECT pipedrive_updated_at FROM biens WHERE pipedrive_deal_id = $1",
        [deal.id],
    );
    if (
        existing[0] &&
        existing[0].pipedrive_updated_at &&
        deal.update_time &&
        deal.update_time < existing[0].pipedrive_updated_at
    ) {
  ...
  `,
        [
            ...
            toBool(g(KEYS.cave)),
            g(KEYS.description) || g(KEYS.descriptif),
            photo1,
            photo2,
            photo3,
            g(KEYS.photo_4),
            ...
            g(KEYS.point_vigilance),
            g(KEYS.points_positifs),
...
async function syncSingleAcquereur(deal) {
    ...
    const { rows: existing } = await pool.query(
        "SELECT pipedrive_updated_at FROM acquereurs WHERE pipedrive_deal_id = $1",
        [deal.id],
    );
    if (
        existing[0] &&
        ...
        logger.info(
            `⚡ Webhook: acquéreur #${deal.id} ignoré (événement obsolète)`,
        );
        return;
    }
  ...
  `,
    ...
    if (acq) {
        ...
        if (rawOcc) {
            ...
            const labels = [
                ...new Set(
                    ids
                        .map((id) => OCC_MAP_SIMPLE[parseInt(id)])
                        .filter(Boolean),
                ),
            ];
    ...
    `,
            [
                acq.id,
                toFloat(g(ACQ_KEYS.budget_min)),
                toFloat(g(ACQ_KEYS.budget_max)),
                toFloat(g(ACQ_KEYS.rentabilite_min)),
                occLabels,
                occIds,
                secteurs || null,
                toFloat(g(ACQ_KEYS.apport)),
                condPretLabel,
            ],
        );
    }
