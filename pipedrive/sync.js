...
async function syncBiens(
  ...
  const KEYS = {
    ...
    nb_pieces:
      findKey("Nombre de pièces") || findKey("Pièces") || findKey("pieces"),
  ...
  try {
    ...
    for (const deal of allDeals) {
      ...
      const toFloat = (v) =>
        v ? parseFloat(String(v).replace(/[^0-9.]/g, "")) || null : null;
      const toBool = (v) =>
    ...
    for (const deal of allDeals) {
      ...
      if (acq) {
        ...
        if (rawOcc) {
          ...
          const labels = [
            ...new Set(ids.map((id) => OCC_MAP[parseInt(id)]).filter(Boolean)),
          ];
