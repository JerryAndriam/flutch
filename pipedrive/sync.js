...
async function syncBiens(
    apiToken,
    stageName = "Commercialisé",
    userId = null,
    pipelineName = null,
) {
    ...
    if (pipelineName) {
        ...
        if (!pipelineIdFilter)
            logger.warn(
                `⚠️ Pipeline "${pipelineName}" introuvable, recherche de l'étape dans tous les pipelines`,
            );
    }
    ...
    if (!stageId)
        throw new Error(
            `Étape "${stageName}" introuvable${pipelineName ? ` dans le pipeline "${pipelineName}"` : ""}`,
        );

    const fieldMap = await getDealFieldsMap(apiToken);
    const findKey = (name) =>
        Object.entries(fieldMap).find(([, v]) => norm(v) === norm(name))?.[0];
    ...
    const KEYS = {
        ...
        dpe:
            findKey("DPE") ||
            findKey("Classe énergie") ||
            findKey("Classe energétique"),
    };
    ...
    while (moreItems) {
        const data = await pdGet(
            `/deals?stage_id=${stageId}&status=open&start=${start}&limit=${limit}`,
            apiToken,
        );
    ...
    try {
        ...
        for (const deal of allDeals) {
            ...
            const toFloat = (v) =>
                v
                    ? parseFloat(String(v).replace(/[^0-9.]/g, "")) || null
                    : null;
            const toBool = (v) =>
                v === true || v === "Oui" || v === "1" || v === 1 ? 1 : 0;

            const cp = g(KEYS.code_postal) || null;
            const rawOcc = g(KEYS.occupation);
            const occIdStr = rawOcc
                ? String(rawOcc).split(",")[0].trim()
                : null;
            ...
            const resolvedModalite = resolveSet(
                g(KEYS.modalite_augmentation),
                MODALITE_LABELS,
            );
            const resolvedTva = resolveEnum(
                g(KEYS.assujettissement_tva),
                TVA_LABELS,
            );
            const resolvedImputTF = resolveSet(
                g(KEYS.imputation_taxe_fonciere),
                IMPUT_TF_LABELS,
            );
            const descriptif = g(KEYS.descriptif) || null;
            const lienDrive = g(KEYS.lien_drive) || null;
            const photoCouv =
                g(KEYS.photo_couverture) || g(KEYS.photo_1) || null;
        ...
        INSERT INTO biens (pipedrive_deal_id, titre, adresse, code_postal, ville, prix_fai, rentabilite,
          ...
          dpe, pipedrive_updated_at, pipedrive_created_at, synced_at)
      ...
      `,
                [
                    deal.id,
                    deal.title || "",
                    g(KEYS.adresse) || deal.title,
                    cp,
                    g(KEYS.ville),
                    toFloat(g(KEYS.prix_fai)) || toFloat(deal.value),
                    null,
                    toFloat(g(KEYS.rentabilite_post_rev)),
                    occLabel,
                    occId ? String(occId) : null,
                    mandatId,
                    toFloat(g(KEYS.surface_totale)) || toFloat(g(KEYS.surface)),
                    g(KEYS.nb_pieces) ? parseInt(g(KEYS.nb_pieces)) : null,
                    g(KEYS.type_bien),
                    g(KEYS.etage) ? parseInt(g(KEYS.etage)) : null,
                    toBool(g(KEYS.ascenseur)),
                    toBool(g(KEYS.balcon)),
                    toBool(g(KEYS.terrasse)),
                    toBool(g(KEYS.jardin)),
                    toBool(g(KEYS.parking)),
                    toBool(g(KEYS.cave)),
                    descriptif,
                    photoCouv,
                    photo2,
                    ...
                    isDeleg,
                    stageName,
                    deal.user_id?.id || null,
                    deal.user_id?.email || null,
                    deal.user_id?.name || null,
                    toFloat(g(KEYS.taxe_fonciere)),
                    toFloat(g(KEYS.charge_annuelle)),
                    toFloat(g(KEYS.loyer_net_bailleur)),
                    g(KEYS.prise_effet_bail) || null,
                    toFloat(g(KEYS.loyer_post_revision)),
                    resolvedTva,
                    resolvedModalite,
                    g(KEYS.point_vigilance) || null,
                    g(KEYS.points_positifs) || null,
                    toFloat(g(KEYS.surface_rdc)),
                    toFloat(g(KEYS.surface_etage)),
                    toFloat(g(KEYS.surface_sous_sol)),
                    toFloat(g(KEYS.surface_ponderee)),
                    resolvedImputTF,
                    toFloat(g(KEYS.rentabilite_actuelle)),
                    lienDrive,
                    resolveEnum(g(KEYS.dpe), DPE_LABELS),
    ...
    if (currentIds.length) {
        ...
        const archResult = await pool.query(
            "UPDATE biens SET archived = 1 WHERE archived = 0",
        );
        archivedCount = archResult.rowCount;
    }
    ...
    await pool.query(
        ...
        [
            ...
            "ok",
            allDeals.length,
            `${stageName} · ${archivedCount} archivés`,
        ],
    );
    if (userId)
        await log(userId, "sync_biens", "sync", null, {
            count: allDeals.length,
            archived: archivedCount,
        });
    logger.info(
        `✅ ${allDeals.length} biens synchronisés · ${archivedCount} archivés`,
    );
    return allDeals.length;
}

async function syncAcquereurs(
    apiToken,
    pipelineNameOrId,
    userId = null,
    stageName = null,
    ...
    let pipelineId = null;
    if (
        typeof pipelineNameOrId === "number" ||
        /^\d+$/.test(pipelineNameOrId)
    ) {
    ...
    if (!pipelineId) {
        logger.warn(
            `⚠️ Pipeline "${pipelineNameOrId}" introuvable, sync annulée`,
        );
        return 0;
    }
    ...
    if (stageName) {
        ...
        if (!stageIdFilter) {
            logger.warn(
                `⚠️ Étape "${stageName}" introuvable dans le pipeline, fallback sur tout le pipeline`,
            );
        }
    }

    while (moreItems) {
        const filterParam = stageIdFilter
            ? `stage_id=${stageIdFilter}`
            : `pipeline_id=${pipelineId}`;
        const data = await pdGet(
            `/deals?${filterParam}&status=open&start=${start}&limit=${limit}`,
            apiToken,
        );
        if (!data?.data?.length) break;
        allDeals = allDeals.concat(data.data);
    ...
    try {
      ...
      `,
            ...
            const { rows: acqRows } = await client.query(
                "SELECT id FROM acquereurs WHERE pipedrive_deal_id = $1",
                [deal.id],
            );
            const acq = acqRows[0];
            if (acq) {
                ...
                const condPretLabel =
                    rawCondPret === 320 || rawCondPret === "320"
                        ? "Oui"
                        : rawCondPret === 321 || rawCondPret === "321"
                          ? "Non"
                          : null;
          ...
          INSERT INTO acquereur_criteria (acquereur_id, budget_min, budget_max, rentabilite_min,
            occupation_status, occupation_ids, secteurs, apport, condition_pret,
            dpe_min, dpe_max, updated_at)
          ...
          ON CONFLICT(acquereur_id) DO UPDATE SET
            ...
            dpe_min=EXCLUDED.dpe_min, dpe_max=EXCLUDED.dpe_max,
            updated_at=NOW()
        `,
                    [
                        acq.id,
                        toFloat(g(KEYS.budget_min)),
                        toFloat(g(KEYS.budget_max)),
                        toFloat(g(KEYS.rentabilite_min)),
                        occLabels,
                        occIds,
                        secteurs || null,
                        toFloat(g(KEYS.apport)),
                        condPretLabel,
                        resolveEnum(g(KEYS.dpe_min), DPE_LABELS),
                        resolveEnum(g(KEYS.dpe_max), DPE_LABELS),
                    ],
                );
            }
        }
    ...
    if (currentIds.length) {
        ...
        if (syncedStageId) {
            const archResult = await pool.query(
                "UPDATE acquereurs SET archived = 1 WHERE archived = 0 AND pipedrive_stage_id = $1",
                [syncedStageId],
            );
            archivedCount = archResult.rowCount;
        } else {
            const archResult = await pool.query(
                "UPDATE acquereurs SET archived = 1 WHERE archived = 0",
            );
            archivedCount = archResult.rowCount;
        }
    }
    ...
    if (userId)
        await log(userId, "sync_acquereurs", "sync", null, {
            count: allDeals.length,
            archived: archivedCount,
        });
    logger.info(
        `✅ ${allDeals.length} acquéreurs synchronisés · ${archivedCount} archivés`,
    );
    return allDeals.length;
}
