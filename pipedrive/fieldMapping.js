...
const MODALITE_LABELS = {
    369: "Révision triennale légale",
    370: "Indexation conventionnelle annuelle",
    371: "ILC",
    372: "ICC",
    373: "Autre",
    463: "Murs libres",
...
const IMPUT_TF_LABELS = {
    195: "Imputée au locataire",
    196: "50/50",
    197: "Non imputée au locataire",
    464: "Murs libres",
};
...
const ACQ_KEYS = {
    ...
    dpe_min: "dpe_min_key_here",
    dpe_max: "dpe_max_key_here",
};
...
async function getBienKeys(apiToken) {
    ...
    if (cachedBienKeys && Date.now() - cachedBienKeysAt < CACHE_TTL_MS)
...
function resolveSet(raw, labels) {
    ...
    return String(raw)
        .split(",")
        .map((id) => labels[parseInt(id.trim())] || "")
        .filter(Boolean)
        .join(", ");
}
